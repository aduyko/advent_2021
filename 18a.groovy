...
// Looking up some answers after the fact, it looks like most people just kept the input as a string and managed that, just counting the distance between numbers for left/right modifications. So this is a little over engineered
inputFile = "inputs/18.txt"
input = []

new File(inputFile).eachLine { line ->
  input << parseInput(line[1..-2], null)
}
...
sum = input[0]
(1..<input.size()).each { row ->
  sum = add(sum, input[row])
}
